...
app.config['MYSQL_USER'] = 'daydr'
app.config['MYSQL_PASSWORD'] = 'Admin!23'
...
def index():
    ...
    tshirt = cur.fetchall()
    print("Check here : {}".format(tshirt))
    values = 'mobiles'
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    wallet = cur.fetchall()
    values = 'television'
...
class RegisterForm(Form):
    ...
    password = PasswordField('', [validators.length(min=3)],
                             render_kw={'placeholder': 'Password'})
    mobile = StringField('', [validators.length(min=10)], render_kw={'placeholder': 'Mobile'})
...
@app.route('/washingmachine', methods=['GET', 'POST'])
def tshirt():
    form = OrderForm(request.form)
    # Create cursor
    cur = mysql.connection.cursor()
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('washingmachines.html', tshirt=products, form=form)
...
def wallet():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('mobiles.html', wallet=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if save_photo:
                    ...
                    if category == 'washingmachine':
                        level = request.form.getlist('washingmachine')
                        for lev in level:
                            yes = 'yes'
                        ...
                        level = request.form.getlist('mobiles')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
